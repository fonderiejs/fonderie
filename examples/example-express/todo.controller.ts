...
import type { ExpressRequest }    from '@fonderie/adapter-express';
...
export function todoController(model: ReturnType<typeof TodoModel>) {
	return {
		...
		async complete(req: Request, res: Response) {
			const { user } = (req as ExpressRequest)._fonderie!
			const todo = await model.complete(req.params['id'] as string, user!.id)
		...
		async remove(req: Request, res: Response) {
			const { user } = (req as ExpressRequest)._fonderie!
			const deleted = await model.remove(req.params['id'] as string, user!.id)
