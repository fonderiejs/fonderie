...
- [ ] **[medium] `IRegisterResult / ILoginResult` — requiresVerification** (client-missing-field)
...
- [ ] **[medium] `IUserDTO (exportMe profile)` — isPhoneVerified** (never-populated)
...
- [ ] **[medium] `IUserPreferences` — dateFormat, timeFormat (and notifications/emailDigest shape)** (type-mismatch)
