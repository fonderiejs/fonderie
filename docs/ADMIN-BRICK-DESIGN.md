...
**Host binding — optional (SHIPPED, admin 0.8.0).**
`AdminModule({ host: 'admin.example.com' })`, or a list: the surface answers
only for that hostname, else the same 404 an unmounted surface gives — not a
403, which would confirm it exists. Covers the served dashboard's unguarded
assets too. Several domains → one deployment works on Vercel, so an operator
can put SSO or an allowlist in front of one hostname with no second service.

The justification narrowed once it met the platform. Its real, concrete win
is that every deployment gets a permanent URL of its own
(`project-abc123.vercel.app`) which answers whatever the custom domain
answers — walking around anything placed in front of that domain. Binding
closes it. What it does NOT do is stop someone sending the right `Host` to
the origin directly; only the edge can (a WAF rule on the path, an origin
locked to the proxy). So it is documented as a hook for a boundary, not a
boundary — the care `trustProxy` gets in core. Refused requests are logged
(the caller learns nothing from a 404; the operator learns something) and
the manifest reports the binding.
