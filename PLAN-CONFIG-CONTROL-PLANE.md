...
on **`@fonderie/config` first** â prove the pattern end-to-end, then generalize
to a control-plane primitive other admin resources (courier templates, â¦) reuse.
...
- â **CRUD** â `setConfigEntry` (upsert) / `getConfigEntry` / `listConfigEntries`
  / `deleteConfigEntry` are already exported.
- â **Multi-env** â `environment` column (`'all'` base + env-specific override,
  resolved in `manager.ts`). Keep as-is; it's our **namespaces**.
- â **Version index, revisions, rollback** â none today (row overwritten; only
  `updated_at`).
- â **Optimistic concurrency** â last-write-wins.
- â **Actor / audit** â no `updated_by`, no audit trail.
- â **Push propagation** â the manager **polls a snapshot every 30s** and never
  touches the (excellent) event bus.
- â **Secret kind** â no masked/encrypted counterpart.
- â **Safe HTTP surface** â `ConfigModule.install` adds no routes.
...
- `fonderie_config_revisions` (new): `key, environment, value, version, actor,
  created_at` â append-only history, one row per committed version.
...
- `current == N` â commit `N+1`; **first writer wins**.
- `current != N` â **conflict**, resolved by a **per-resource policy**:
  - **automation (config values, the CLI/LLM writer) â reject-and-retry**
    (409 + current version); the AI re-reads and re-applies. No human draft to
    lose â simplest and always correct. *This is config's default.*
  - **human-authored docs (templates, later) â preserve-both + warn** (save as
    `N+2`, keep both, human reconciles). Same version primitive, different policy.

## 3. Event propagation (dual channel â the key design)
...
- **Durable event `config.changed`** (payload: `key, env, version, actor` â never
  the value; never anything for secrets) â **`audit`** (who changed what) and
  **`webhooks`** (external fan-out) consume it once each, guaranteed.
- **Broadcast invalidation** â **every** config manager wakes and **re-reads** its
  snapshot (NOTIFY is wake-only; re-read is staleness-proof). Missing one is
  harmless â the 1s floor catches it. Ordering is safe because a re-delivered
  older `version` is ignored (Â§2's index doubles as the idempotency guard).
...
Same lifecycle machinery (Â§1â3), different **exposure** â separate table + read
path so a masking bug in config physically can't leak secrets:
- `fonderie_secrets` (+ `fonderie_secret_revisions`) â mirrors config's shape.
...
- **Tighter authz** â a separate admin permission from config.
- **Never** in `config.changed`-style event payloads or logs.
- **No package rename** â `@fonderie/config` stays; it's the runtime-settings
...
`POST /admin/config/:key/rollback` â and `/admin/secrets/*` (masked, tighter).
Every mutation carries the actor and emits `config.changed`.
- **Open decision â admin auth is greenfield** (no admin role today): start with a
...
## 6. CLI â the LLM interface (management-CLI-first)
...
- `fonderie config apply <file>` â declarative, diffable, GitOps-able (k8s
  `apply`) â **phase 2**; imperative `set` first.
...
1. **Version index + revisions + rollback + optimistic concurrency** (data model +
   services, config only). The heart of the pattern â no HTTP yet, proven against
   real Postgres.
2. **Event propagation** â durable `config.changed` + broadcast invalidation;
...
2. **Admin auth** â bootstrap token vs full permission model for v1. *Lean:
   bootstrap token, grow into permissions.*
3. **Secret encryption provider** â plaintext-in-separate-table + masking for v1,
   or pluggable encryptor from day one. *Lean: masking + pluggable hook (no-op
   default) so encryption can drop in.*
4. **Config conflict default** â confirm reject-and-retry for config values
   (automation). *Lean: yes.*
...
machinery â the `checkReadiness()`/contribution pattern applied to management.
