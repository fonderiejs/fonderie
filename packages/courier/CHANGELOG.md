...
- c5f4f50: Add `checkSenderDns` — verify the sending domain actually authorises us
...
- 52d8a7a: Add `replyTo` to the email channel
...
- f1095a6: Email is branded with the app's name, not the framework's
...
- 081d8fd: Catch an undeliverable message type at BOOT instead of at the first user who
  triggers it.
...
- 0d71572: Move the serverless queue plumbing into the bricks, where it belongs.
...
- 7ec61eb: Await the message-log status writes, so a send's outcome is not lost on serverless.
...
- 27fce4d: Support nodemailer 10. Its `SendMailOptions` is now an exact-optional type, so the SMTP channel no longer passes `html: undefined` for text-only templates — it omits the field instead (never meaningful at runtime). Bumps the dependency to `^10.0.0`; nodemailer 10 requires Node ≥ 20, which the package already requires.
