...
const GOOD = {
	'email.acme.example': ['v=spf1 include:_spf.example.net ~all'],
	'_dmarc.email.acme.example': ['v=DMARC1; p=quarantine; rua=mailto:d@acme.example'],
};

test('a correctly configured sending domain reports ok', async () => {
	const r = await checkSenderDns('hello@email.acme.example', { resolveTxt: zone(GOOD) });
	assert.equal(r.ok, true);
	assert.equal(r.domain, 'email.acme.example');
...
test('a display-name from address still yields the domain', async () => {
	assert.equal(senderDomain('Acme <hello@email.acme.example>'), 'email.acme.example');
	assert.equal(senderDomain('hello@email.acme.example'), 'email.acme.example');
	assert.equal(senderDomain('hello@Email.Acme.example.'), 'email.acme.example');
...
test('absent SPF with an aligned DKIM key is ADVICE, not a failure', async () => {
	const r = await checkSenderDns('hello@email.acme.example', {
		dkimSelectors: ['resend'],
		resolveTxt: zone({
			'_dmarc.acme.example': ['v=DMARC1; p=reject'],
			'resend._domainkey.email.acme.example': ['v=DKIM1; k=rsa; p=MIIBIjANBg'],
		}),
	});
...
test('absent SPF AND absent DKIM IS a failure — nothing can align', async () => {
	const r = await checkSenderDns('hello@email.acme.example', {
		dkimSelectors: ['resend'],
		resolveTxt: zone({ '_dmarc.acme.example': ['v=DMARC1; p=reject'] }),
	});
...
test('returnPathDomain checks SPF where it actually applies', async () => {
	const r = await checkSenderDns('hello@email.acme.example', {
		returnPathDomain: 'send.email.acme.example',
		dkimSelectors: ['resend'],
		resolveTxt: zone({
			'send.email.acme.example': ['v=spf1 ip4:52.3.252.119 ~all'],
			'_dmarc.acme.example': ['v=DMARC1; p=reject'],
			'resend._domainkey.email.acme.example': ['v=DKIM1; k=rsa; p=MIIBIjANBg'],
		}),
	});
	...
	assert.equal(spf.foundAt, 'send.email.acme.example');
...
test('TWO SPF records are worse than one — RFC 7208 makes it a permerror', async () => {
	...
	const r = await checkSenderDns('hello@email.acme.example', {
		resolveTxt: zone({
			...GOOD,
			'email.acme.example': ['v=spf1 include:a ~all', 'v=spf1 include:b ~all'],
		}),
	});
	assert.equal(r.ok, false);
...
test('DMARC inherited from the parent is CORRECT, not a finding', async () => {
	...
	assert.equal(dmarc.foundAt, 'acme.example', 'reports WHERE it was found');
});

test('DMARC missing everywhere up the chain is reported', async () => {
	const r = await checkSenderDns('hello@email.acme.example', {
		resolveTxt: zone({ 'email.acme.example': ['v=spf1 -all'] }),
	});
	assert.equal(r.ok, false);
...
test('p=none is flagged as monitoring-only, not silently accepted', async () => {
	const r = await checkSenderDns('hello@email.acme.example', {
		resolveTxt: zone({
			'email.acme.example': ['v=spf1 -all'],
			'_dmarc.email.acme.example': ['v=DMARC1; p=none; rua=mailto:d@x.com'],
		}),
	});
	const dmarc = r.records.find((x) => x.kind === 'dmarc')!;
	assert.equal(dmarc.present, true);
...
test('a TXT record split into chunks is joined, not truncated', async () => {
	...
	const split: ResolveTxt = async (host) =>
		host === 'email.acme.example'
			? [['v=spf1 include:_spf.exam', 'ple.net ~all']]
			: host === '_dmarc.email.acme.example'
				? [['v=DMARC1; p=reject']]
				: (() => {
						throw new Error('ENOTFOUND');
					})();
	const r = await checkSenderDns('hello@email.acme.example', { resolveTxt: split });
	assert.equal(r.ok, true);
	assert.equal(
...
test('senderDnsCheck: the doctor check over an email channel, resolver injected', async () => {
	...
	const check = senderDnsCheck({ from: 'Hi <hello@email.acme.example>' }, zone(GOOD));
	assert.equal(check.name, 'courier.sender-dns');
	const good = await check.run();
	assert.equal(good.ok, true);
	const bad = await senderDnsCheck({ from: 'hello@email.acme.example' }, zone({})).run();
