...
- 7bee9c2: Stripe API pin moves from `2024-11-20.acacia` to `2026-08-26.dahlia`
...
- 09e4808: The webhook version-drift advice stops claiming data is being lost
...
- 9bd6566: Describe the money reads for `@fonderie/admin`
...
- e687c4e: Describe the admin routes for `@fonderie/admin`; inline `/plans` writes and `/billing/wallet/grant` deprecated
...
- 9498067: Add `checkSubscriptionDrift`, and a way to read a subscription back
...
- 3f3867a: Fix invoice webhooks silently dropping under a 2025+ endpoint API version, and detect the version gap
...
- e2e1048: Add `checkPriceConsistency` — reconcile the catalog's prices against the provider's
...
- f6d6c98: Fix blank purchase receipts from the checkout and auto-recharge paths
...
- c896e5f: Warn when an event reaches the wrong webhook endpoint
...
- 0cabc68: Detect webhook events that were never registered
...
- ad3a8f3: Stop a subscription webhook from 500ing when the provider carries no subscriber metadata
...
- a9e8d76: Add `webhookStats()` — whether the payment provider is actually reaching this deployment.
