...
function toReport<R extends IReportLike>(report: R, describe: (r: R) => string[]): IAdminCheckReport {
	if (report.unsupported) return { ok: true, findings: [], skipped: 'the provider cannot be asked' };
...
export function describeBillingAdminChecks(store: IStoreAdapter, config: IBillingConfig): IAdminCheck[] {
	const { provider } = config;
	const checks: IAdminCheck[] = [
		{
			name: 'billing.price-consistency',
			run: async () => toReport(await checkPriceConsistency(provider, config), describePriceProblems),
		},
		{
			name: 'billing.subscription-drift',
			run: async () => toReport(await checkSubscriptionDrift(provider, store), describeSubscriptionDrift),
		},
	];
