 ...
 *
 * On native this is what decides whether to render Sign in with Apple at all.
 * Apple's Guideline 4.8 makes that button's presence conditional on the other
 * social options, so guessing is a review risk as well as a broken button —
 * and the server is the only side that knows which providers have credentials.
 *
 * Carries no token storage, so it is identical to the react-auth hook; it
 * lives here because this package exports its hooks explicitly rather than
 * re-exporting react-auth wholesale.
 */
