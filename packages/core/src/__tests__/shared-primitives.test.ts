...
test('secretStrengthProblem: too-short / placeholder / strong', () => {
	...
	assert.equal(
		secretStrengthProblem('admin-token-admin-token-admin-token'),
		'placeholder',
		'unified denylist includes admin-token',
	);
	assert.equal(
		secretStrengthProblem('S3cure-random-secret-9f3a1c7e2b8d40x'),
		null,
		'strong → null',
	);
});
...
test('encode/decodeKeysetCursor: round-trips; range-checks; rejects junk', () => {
	...
	assert.equal(
		decodeKeysetCursor(Buffer.from(`["2026-13-40T25:61:99Z","${uuid}"]`).toString('base64url')),
		null,
	);
	assert.equal(
		decodeKeysetCursor(Buffer.from(`["2026-01-01T00:00:00Z","not-a-uuid"]`).toString('base64url')),
		null,
	);
