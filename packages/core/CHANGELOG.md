...
- 38f2410: Export `normalizeRequestPath` and `normalizeMountPath` — one definition each
...
- 2363ea6: `describeAdmin().checks` — a module offers reconciliation checks to the doctor
...
- e687c4e: `describeAdmin()` — a module offers its operator routes; the app collects them
...
- 981ee15: Modules can report their version; `securityReport()` carries it
...
- d470d85: Reserve a route prefix, and list the route table
