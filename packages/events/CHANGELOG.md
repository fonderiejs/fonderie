...
- 0205f12: `runWorker(...).stop()` can no longer hang forever, and always releases the port.
...
- f8f32ea: `PGTransport.stop()` now releases the connection pool it opened.
...
- 83fdb67: Offer the audit-chain integrity check to the doctor
...
- 2363ea6: Offer the outbox check to the doctor
...
- a7e9fd5: `runWorker` — a queue consumer that deploys anywhere, so choosing a host is not
  a code change.
